...
macro TimeLapseAnalysis2D
{
	...
	Dialog.create("2D TimeLapse Analysis");
		Dialog.setInsets(-5, 0, 0);
	    Dialog.addMessage("This function will perform morphological and networking analysis on a thresholded 2D TimeLapse (XYT) stack.");
	...
	for (i=1;i<=frames;i++)
	{
		...
		//{			
			if (Channel_mask!="None")
			{
				...
				Stack.setFrame(i);
				run("Duplicate...", "duplicate range=" + i);
		...
		selectWindow(input);
		Stack.setFrame(i);
		if (Stack.isHyperstack==false) run("Duplicate...", "duplicate range=" + i + "-" + i);
		if (Stack.isHyperstack==true)
		{
			Stack.getPosition(c,s,f);
			run("Duplicate...", "duplicate channels=" + c + "-" + c + " frames=" + s + "-" + s);
		}
		name = input + " Frame: " + i;
		rename(name);
